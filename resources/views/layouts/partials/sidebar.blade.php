<div class="sidenav-section" data-simplebar>
    <!-- SideNav Inner Start -->
    <div class="sidenav-inner">
        <nav class="sidenav-menu" id="sidenav-menu">
            <ul>
                <li class="has-sub-menu">
                    <a href="{{ route('user.home') }}"><i data-feather="home" class="nav-icon"></i>Home</a>
                </li>
                <li class="has-sub-menu">
                    <a href="#"><i data-feather="home" class="nav-icon"></i>Appeals<span class="arrow"><i class="fa fa-angle-down"></i></span></a>
                    <ul class="sub-menu">
                        <li><a href="{{ route('user.operationals.index') }}">Operationals</a></li>
                        <li><a href="{{ route('user.dsngs.create') }}">DSNG</a></li>
                    </ul>
                </li>
            </ul>
        </nav>
    </div>
    <!-- SideNav Inner End -->
</div>
